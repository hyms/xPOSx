      ...
      <q-toolbar>
        ...
        <div class="row items-center no-wrap q-gutter-sm">
          ...
          <q-btn 
            ...
            round 
            dense 
            :icon="($q.fullscreen && $q.fullscreen.isActive) ? 'fullscreen_exit' : 'fullscreen'" 
          ...
          <q-btn flat no-caps>
            ...
            <div class="text-subtitle2 gt-xs">{{ authStore.username }}</div>
            ...
            <q-menu transition-show="jump-down" transition-hide="jump-up">
              <q-list style="min-width: 180px">
                ...
                <q-item clickable v-close-popup to="/settings" v-if="authStore.hasPermission('settings_view')">
    ...
    >
      <q-scroll-area class="fit">
        <q-list padding>
          ...
          <q-item v-if="authStore.hasPermission('pos_view')" clickable to="/pos" exact active-class="active-item" class="pos-cta-btn">
            <q-item-section avatar>
              <q-icon name="point_of_sale" />
            </q-item-section>
            <q-item-section class="text-weight-bold">VENTA POS</q-item-section>
          </q-item>
          ...
          >
            <q-item v-if="authStore.hasPermission('sales_view')" clickable to="/sales" active-class="active-item">
              <q-item-section avatar><q-icon name="list_alt" /></q-item-section>
              <q-item-section>Ventas</q-item-section>
            </q-item>
            <q-item v-if="authStore.hasPermission('quotations_view')" clickable to="/quotations" active-class="active-item">
              <q-item-section avatar><q-icon name="request_quote" /></q-item-section>
              <q-item-section>Cotizaciones</q-item-section>
            </q-item>
            <q-item v-if="authStore.hasPermission('expenses_view')" clickable to="/expenses" active-class="active-item">
              <q-item-section avatar><q-icon name="payments" /></q-item-section>
              <q-item-section>Gastos</q-item-section>
            </q-item>
            <q-item v-if="authStore.hasPermission('returns_view')" clickable to="/returns" active-class="active-item">
          ...
          <q-expansion-item
            ...
            header-class="text-weight-medium"
          >
            <q-item v-if="authStore.hasPermission('products_view')" clickable to="/products" active-class="active-item">
              <q-item-section avatar><q-icon name="inventory" /></q-item-section>
              <q-item-section>Productos</q-item-section>
            </q-item>
            <q-item v-if="authStore.hasPermission('purchases_view')" clickable to="/purchases" active-class="active-item">
              <q-item-section avatar><q-icon name="shopping_cart" /></q-item-section>
              <q-item-section>Compras</q-item-section>
            </q-item>
            <q-item v-if="authStore.hasPermission('transfers_view')" clickable to="/transfers" active-class="active-item">
              <q-item-section avatar><q-icon name="swap_horiz" /></q-item-section>
              <q-item-section>Transferencias</q-item-section>
            </q-item>
            <q-item v-if="authStore.hasPermission('adjustments_view')" clickable to="/adjustments" active-class="active-item">
              <q-item-section avatar><q-icon name="tune" /></q-item-section>
              <q-item-section>Ajustes de Stock</q-item-section>
            </q-item>
            <q-item v-if="authStore.hasPermission('categories_view')" clickable to="/categories" active-class="active-item">
              <q-item-section avatar><q-icon name="category" /></q-item-section>
              <q-item-section>Categorías</q-item-section>
            </q-item>
            <q-item v-if="authStore.hasPermission('units_view')" clickable to="/units" active-class="active-item">
          ...
          <q-expansion-item
            ...
            header-class="text-weight-medium"
          >
            <q-item v-if="authStore.hasPermission('clients_view')" clickable to="/clients" active-class="active-item">
              <q-item-section avatar><q-icon name="person_add" /></q-item-section>
              <q-item-section>Clientes</q-item-section>
            </q-item>
            <q-item v-if="authStore.hasPermission('providers_view')" clickable to="/providers" active-class="active-item">
          ...
          <q-expansion-item
            ...
            header-class="text-weight-medium"
          >
            <q-item v-if="authStore.hasPermission('users_view')" clickable to="/users" active-class="active-item">
              <q-item-section avatar><q-icon name="people" /></q-item-section>
              <q-item-section>Usuarios</q-item-section>
            </q-item>
            <q-item v-if="authStore.hasPermission('roles_view')" clickable to="/roles" active-class="active-item">
            ...
            <q-item v-if="authStore.hasPermission('warehouses_view')" clickable to="/warehouses" active-class="active-item">
              <q-item-section avatar><q-icon name="store" /></q-item-section>
              <q-item-section>Almacenes</q-item-section>
            </q-item>
          </q-expansion-item>

          <q-item v-if="authStore.hasPermission('reports_view')" clickable to="/reports" active-class="active-item">
            <q-item-section avatar><q-icon name="bar_chart" /></q-item-section>
            <q-item-section>Reportes</q-item-section>
          </q-item>

          <q-item v-if="authStore.hasPermission('settings_view')" clickable to="/settings" active-class="active-item">
...
const hasOperationsPermission = computed(() => {
  return ['sales_view', 'quotations_view', 'expenses_view', 'returns_view']
    .some(p => authStore.hasPermission(p))
})

const hasInventoryPermission = computed(() => {
  return ['products_view', 'purchases_view', 'transfers_view', 'adjustments_view', 'categories_view', 'units_view']
    .some(p => authStore.hasPermission(p))
})

const hasContactsPermission = computed(() => {
  return ['clients_view', 'providers_view']
    .some(p => authStore.hasPermission(p))
})

const hasAdminPermission = computed(() => {
  return ['users_view', 'roles_view', 'permissions_view', 'warehouses_view']
    .some(p => authStore.hasPermission(p))
})
