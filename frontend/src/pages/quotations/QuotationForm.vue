    ...
    <q-page padding>
        <q-card>
            <q-card-section>
                ...
                >
                    {{ isEdit ? "Ver Cotización" : "Crear Nueva Cotización" }}
                </div>
            </q-card-section>
...
const addProduct = (val: any) => {
    ...
    const existing = formData.details.find(
        (d: QuotationDetail) => d.productId === product.id,
    );
    if (existing) {
        existing.quantity++;
        updateRow(existing);
...
const removeProduct = (row: any) => {
    formData.details = formData.details.filter(
        (d: QuotationDetail) => d.productId !== row.productId,
    );
    calculateTotal();
};

const calculateTotal = () => {
    formData.grandTotal = formData.details.reduce(
        (acc: number, curr: QuotationDetail) => acc + curr.total,
        0,
    );
};
...
const submitForm = async () => {
    if (formData.details.length === 0) {
        $q.notify({
            color: "warning",
            message: "Debe añadir al menos un producto",
        });
        return;
    }
    saving.value = true;
...
onMounted(async () => {
    ...
    clientOptions.value = clients.data.map((c) => ({
        label: c.name,
        value: c.id,
    }));
    warehouseOptions.value = warehouses.data.map((w) => ({
        label: w.name,
        value: w.id,
    }));
    ...
    if (quotationId) {
        ...
        try {
            ...
            $q.notify({
                color: "negative",
                message: "Error al cargar la cotización para edición",
            });
            router.push("/quotations");
        }
    }
