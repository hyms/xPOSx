<template>
  <q-page padding>
    <q-card>
      <q-card-section>
        <div class="text-h6">Registrar Devolución de Venta</div>
      </q-card-section>
        ...
        <q-card-section class="q-gutter-md">
          ...
          <div class="row items-center q-col-gutter-md">
            ...
            <div class="col-12 col-md-2">
              <q-btn color="primary" label="Añadir" @click="addProductToSaleReturn" :disable="!selectedProduct" class="full-width" />
            </div>
          </div>
...
const subTotal = computed(() => formData.details.reduce((acc, item) => acc + (item.quantity * item.price), 0))
...
const onSubmit = async () => {
  ...
  try {
    ...
    await returnService.createSaleReturn(payload as SaleReturn)
    $q.notify({ color: 'positive', message: 'Devolución de Venta registrada con éxito' })
    router.push('/returns')
  } catch (error) {
