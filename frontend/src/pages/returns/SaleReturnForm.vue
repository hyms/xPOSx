...
const formData = reactive<
    Partial<SaleReturn> & { details: SaleReturnDetail[] }
>({
    ref: `SR-${Date.now()}`,
...
const subTotal = computed(() =>
    formData.details.reduce(
        (acc: number, item: SaleReturnDetail) =>
...
const addProductToSaleReturn = () => {
    ...
    const existingDetail = formData.details.find(
        (d: SaleReturnDetail) => d.productId === selectedProduct.value!.id,
    );
    if (existingDetail) {
...
const removeProduct = (row: SaleReturnDetail) => {
    const index = formData.details.findIndex(
        (d: SaleReturnDetail) => d.productId === row.productId,
    );
    if (index > -1) {
...
const onSubmit = async () => {
    ...
    try {
        ...
        $q.notify({
            color: "negative",
            message: "Error al registrar la devolución de venta",
        });
    } finally {
        saving.value = false;
    }
