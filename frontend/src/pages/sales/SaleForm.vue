            ...
            <q-form @submit.prevent="onSubmit">
                ...
                <q-card-actions align="right">
                    ...
                    <q-btn
                        ...
                        :loading="submitting"
                        :disable="
                            submitting || sale.details.length === 0 || isEdit
                        "
                    />
                </q-card-actions>
            </q-form>
...
const subTotal = computed(() =>
    sale.details.reduce(
        (acc: number, item: SaleDetail) => acc + item.quantity * item.price,
        0,
    ),
);
...
const addProductToSale = () => {
    ...
    const existingDetail = sale.details.find(
        (d: SaleDetail) => d.productId === selectedProduct.value!.id,
    );
    if (existingDetail) {
...
const removeProduct = (row: SaleDetail) => {
    const index = sale.details.findIndex(
        (d: SaleDetail) => d.productId === row.productId,
    );
    if (index > -1) {
...
onMounted(async () => {
    ...
    clientOptions.value = clientsRes.data.map((c) => ({
        label: c.name,
        value: c.id,
    }));
    ...
    if (saleId) {
        ...
        try {
            ...
            $q.notify({
                color: "negative",
                message: "Error al cargar la venta para edición",
            });
            router.push("/sales");
        }
    }
