<template>
    <q-page padding>
        <q-card>
            <q-card-section>
                <div class="text-h6">
                    {{ isEdit ? "Ver Transferencia" : "Crear Transferencia" }}
                </div>
            </q-card-section>
                    ...
                    <div class="row justify-between items-center q-mt-md">
                        ...
                        <div class="q-gutter-sm">
                            ...
                            <q-btn
                                ...
                                :loading="saving"
                                :disable="
                                    formData.details.length === 0 || isEdit
                                "
                            />
                        </div>
                    </div>
...
const itemColumns = [
    {
        name: "name",
        label: "Producto",
        field: (row: any) => getProductName(row.productId),
        align: "left" as const,
    },
...
const getProductName = (id: number) =>
    allProducts.value.find((p: Product) => p.id === id)?.name || "Desconocido";

const grandTotal = computed(() =>
    formData.details.reduce(
        (sum: number, item: TransferDetail) => sum + item.total,
        0,
    ),
);
        ...
        if (transferId) {
            ...
            try {
                ...
                $q.notify({
                    color: "negative",
                    message:
                        "Error al cargar la transferencia para visualización",
                });
...
const addProduct = (product: Product) => {
    ...
    const existing = formData.details.find(
        (d: TransferDetail) => d.productId === product.id,
    );
    if (existing) {
        existing.quantity++;
...
const onSubmit = async () => {
    ...
    try {
        ...
        $q.notify({
            color: "positive",
            message: "Transferencia guardada con éxito",
        });
        router.push("/transfers");
    } catch (error) {
        $q.notify({
